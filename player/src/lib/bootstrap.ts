...
// (`<origin>/callback`). The router is hash-based, so on return from
// Discord we rewrite `/callback?code=…` to `/#/callback?code=…` before
// the app mounts — the router then handles it as the existing
// `callback` route without any other changes.
