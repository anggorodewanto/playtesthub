...
| GetADTDownloadInfo | player | `playtestId` | `{downloadUrl, expiresAt?, source ('adt'|'fallback')}` (gated on `applicant.status='APPROVED'` exactly like `GetGrantedCode`) | read-only (each call may mint a fresh URL per ADT semantics) | M5.B |

### 4.8 ADT distribution model — linking flow, per-applicant URL resolution, fallback
...
- Calls `adt.Client.IssueDownloadURL(adtNamespace, adtGameId, adtBuildId, applicantIdent)`. Per-applicant URL is preferred for revocability and audit granularity (ADT can revoke a single tester's download without re-DM'ing the playtest).
- On ADT `401` (linkage flag missing — could be operator-side unlink on ADT or a `union_namespace` claim shift): returns `FailedPrecondition` ("adt linkage no longer exists or service token rejected, re-link required"); applicant stays `PENDING`.
...
- DM body shape (ADT): `Download your playtest build: <url>` — single line, no markdown. `RetryDM` re-mints a fresh URL (the prior one may have expired).
- The `applicant.approve` audit row carries `{adtUrl, adtUrlSource}`. **URL is not redacted** — URLs ≠ codes, and forensics require the URL. The §6 Observability log-redaction rule applies to code values only.
