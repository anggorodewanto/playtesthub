...
10. **E2E golden flow (M1 stop)** — a top-level `e2e/` test that signs up a fake player end-to-end against the real backend + testcontainers Postgres (IAM + Discord faked) and asserts the applicant row lands with `status=PENDING`.
11. **CI + README walkthrough** — every gate in [`engineering.md`](engineering.md) §5 live on the GitHub Actions workflow; README reproduces the golden flow on a clean checkout; MIT `LICENSE` at repo root.
